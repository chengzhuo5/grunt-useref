  ...
  function getBlocks(body) {
    ...
    lines.forEach(function (l) {
      ...
      if (build) {
        ...
        sections[
          [build[1], grunt.template.process(build[2].trim())].join(':')
        ] = last = [];
      }
  ...
  var helpers = {
    ...
    useref_css: function (content, block, target) {
      ...
      return content.replace(
        block,
        indent + '<link rel="stylesheet" href="' + target + '"/>'
      );
    },
    ...
    useref_js: function (content, block, target) {
      ...
      return content.replace(
        block,
        indent + '<script src="' + target + '"></script>'
      );
    },
